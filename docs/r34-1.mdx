...
- Demonstrate licensing changes starting with NGINX Plus R34
...
Starting on R34, there will be two aspects that will be enforced. We will go over workflow details of Entitlement &
Visibility (E&V).
...
NGINX Plus will collect and report following data starting with R34. This data is encrypted and sent from each NGINX instance to
...
![R34 License Options](media/intro/1.png)

From the perspective of the NGINX Plus R34 instance, two endpoints are available for usage reports:
...
For the NGINX Plus R34 release, there will only be one main use case:
...
1. Handing upgrade to NGINX Plus R34
...
Since the JWT is a focus of this release, let's get through the steps of getting it by [clicking here](r34-2.mdx).
