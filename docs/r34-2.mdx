...
The forward proxy is already an existing piece thats already installed. If you are interested you can expand the box
below.
...
Now let's go ahead and configure this instance to use a forward proxy. Add the following line to the **mgmt** section
of `/etc/nginx.conf`:
...
You may also want to increase the interval so NGINX Plus tries to submit the E&V report more often for error log
verbose in this lab. Below is a snippet of setting the **interval** parameter.
...
<Collapsible title="Example NGINX Config">
