...
:bangbang: Be sure your UDF deployment is using at least *SDE NGINX Plus R34 Upskilling (**version 20**)*
...
- Become familiar configuring NGINX Plus R34 with NIM as the usage report endpoint.
...
Let's now proceed to installing NGINX Plus R34 and configuring the usage report endpoint to NIM instead of the default
F5 SaaS endpoint.

The prerequisites for installing NGINX Plus is already complete so all you need to do is install it.

1. From UDF, open up a **Web Shell** for your **NGINX Plus R34 (Part 1)** instance.
...
following **mgmt** section. We will be doing the following:
    ...
    * Use the NIM provided self-signed certificate
    ...
    example of configuring the NGINX R34 so it uses your root CA when usage reports are sent to NIM.

    ```shell
    $ cat /etc/nginx/nginx.conf # NGINX Plus R34 instance
    . . .
    mgmt {
    ...
    <br/>
    <br/>
    So here, the root CA certificate is copied from NIM to the NGINX Plus R34 instance and set using the
...
1. You should now notice the `info` line in `/var/log/nginx/error.log` showing the usage report was sent to NIM.
    ...
    2024/10/31 07:20:58 [notice] 9833#9833: nginx/1.27.2 (nginx-plus-r34)
...
1. On NGINX Plus R34, configure usage report endpoint to NIM
...
the next lab where we will work on a lab where NIM is also disconnected like NGINX Plus. [Click here](r34-5.mdx) to
proceed to that.
