...
![OIDC Diagram](media/OIDCDiagram.png)
...
- To use a _self-signed_ certificate on the Authentication Server, insert a reference to it inside the [`ssl_trusted_certificate`](https://nginx.org/en/docs/http/ngx_http_oidc_module.html#ssl_trusted_certificate) configuration
...
- OAuth Client Secret: **xyz**
  - The Client ID and Client Secret are used as _HTTP Basic_ credentials by NGINX when contacting the `token` endpoint of the AS
...
3. Switch to the _oidc-as_ directory in ubuntu's homedir: `cd /home/ubuntu/oidc-as`
